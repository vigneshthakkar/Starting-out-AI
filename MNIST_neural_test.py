...
while c=='y':
	with tf.Session() as sess:
		...
		while char=='y':
			...
			correct=0
			for predictlabel,testlabel in zip(predictlabels,testlabels):
				greatest,index=predictlabel[0],0
				for i in range(1,10):
					if greatest<predictlabel[i]: greatest,index=predictlabel[i],i
				if testlabel[index]==1: correct+=1
